...
LayoutEngine.__rectanglesToColumns = function (rectArray, gutterY) {
  ...
  // TODO:
  // columns SHOULD contain arrays with all rectangles for each column
  // that way we could later always reduce those to rectangles
...
LayoutEngine.__placeAfterRectangle = function (rectArray, gutterY, collapsing, numColumns, index) {
  ...
  if (collapsing) {
    ...
    // return the tallest column for y
    // return numColumns % index for x

    // return Object.assign({}, columnsByHeightDesc[(index % numColumns)], { x: columns[(index % numColumns)]['x'] })
